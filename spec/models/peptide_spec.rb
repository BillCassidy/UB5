...
describe '#parse_dataline' do
  it 'should write one record for each line that is input' do

    puts "Peptide.count before #{Peptide.count}"

    Peptide.count.should == 1
    perform

    # I guess once it finishes doing things, it doesn't write this second puts
    puts "Peptide.count after #{Peptide.count}"
  end
end
